...
def _create_window_mask(seq_len: int, window_size: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    ...
    return mask
# 位置: 放在 _create_window_mask 函数之后

def _create_graph_mask(edge_index, num_nodes, device, dtype):
    """
    根据 edge_index 创建一个稠密的注意力掩码.
    这个掩码允许节点关注其在图中的直接邻居。
    """
    # 初始化一个完全屏蔽的掩码，所有值为-inf
    mask = torch.full((num_nodes, num_nodes), torch.finfo(dtype).min, 
                      device=device, dtype=dtype)
    
    # 允许节点关注自身 (self-attention)
...
class LongShortAttention(nn.Module):
    ...
    # [新] 生成图邻居掩码
        graph_mask = _create_graph_mask(edge_index, seq_len, x.device, x.dtype)

    # [核心修改] 融合掩码:
    # 节点i可以关注节点j，如果j在i的序列窗口内(值为0) 或 j是i的图邻居(值为0)
    # torch.max(mask1, mask2) 可以完美实现这种 "OR" 逻辑
        short_range_mask = torch.max(window_mask, graph_mask)
    
    # 将批处理掩码加上去，确保不同图的节点间不产生注意力
        if batch_mask is not None:
            attn_mask = short_range_mask + batch_mask
        else:
            attn_mask = short_range_mask
...
class HybridGVPLongShortLayer(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, batch=None):
        ...
        # 修改为 (将 edge_index 传递下去):
        s_attn_out = self.ls_attention(s_reshaped, edge_index=edge_index, batch=batch)
    # --- 结束修改 ---
