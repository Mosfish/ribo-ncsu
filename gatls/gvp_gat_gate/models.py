...
class AutoregressiveMultiGNNv1(torch.nn.Module):
    ...
    def sample(
        ...
        # --- 核心修复区：确保 h_V, h_E, edge_index 都被正确地批处理和扁平化 ---

        # 1. 重复节点特征 (原先正确)
        h_V = (h_V[0].repeat(n_samples, 1), h_V[1].repeat(n_samples, 1, 1))
        
        # 2. 重复并【扁平化】边特征
        #    原 h_E[0] shape: [E, dim_s] -> repeat -> [n_samples * E, dim_s]
        h_E_s_flat = h_E[0].repeat(n_samples, 1)
        #    原 h_E[1] shape: [E, dim_v, 3] -> repeat -> [n_samples * E, dim_v, 3]
        h_E_v_flat = h_E[1].repeat(n_samples, 1, 1)
        h_E_flat = (h_E_s_flat, h_E_v_flat)

        # 3. 正确创建批次化的、扁平的 edge_index (原先已修复)
        edge_index_repeated = edge_index.repeat(1, n_samples)
        offset = num_nodes * torch.arange(n_samples, device=device).repeat_interleave(edge_index.size(1))
        edge_index_batched = edge_index_repeated + offset.unsqueeze(0)
        ...
        for i in range(num_nodes):
            ...
            # 现在所有的索引都是在扁平化的“巨大图”上操作，维度完全匹配
            h_S_ = h_S[edge_index_batched[0]]
            mask = (edge_index_batched[0] % num_nodes) >= (edge_index_batched[1] % num_nodes)
            h_S_[mask] = 0
            
            # --- 核心修复：使用扁平化的 h_E_flat ---
            # torch.cat 的两个输入现在都是 2D 张量，可以正确拼接
            h_E_decoder_s = torch.cat([h_E_flat[0], h_S_], dim=-1)
            h_E_decoder = (h_E_decoder_s, h_E_flat[1])
                      
            edge_mask = (edge_index_batched[1] % num_nodes) == i
            edge_index_ = edge_index_batched[:, edge_mask]
