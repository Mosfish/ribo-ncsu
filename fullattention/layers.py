...
class TransformerConvShortRange(nn.Module):
    """
    使用 TransformerConv 的短程注意力模块 (V4 架构)。
    关键优势: 它接受 `edge_dim` 并利用 `edge_attr` 进行注意力计算。
    """
    def __init__(self, embed_dim, num_heads, dropout=0.1, edge_embed_dim=32):
        super().__init__()
        self.transformer_conv = TransformerConv(
            ...
            concat=False,
            dropout=dropout,
            edge_dim=edge_embed_dim, # <-- V4 关键：利用边特征
            root_weight=True
        )
        self.out_proj = nn.Linear(embed_dim, embed_dim)

    def forward(self, s, edge_index, edge_attr_s):
        """
        s: 节点标量特征 [bsz, seq_len, embed_dim] (bsz=confs, seq_len=nodes)
        edge_index: [2, num_edges]
        edge_attr_s: 边标量特征 [num_edges, bsz (confs), edge_embed_dim]
        """
        bsz, seq_len, embed_dim = s.shape

        # [bsz, seq_len, embed_dim] -> [bsz * seq_len, embed_dim]
        s_reshaped = s.view(-1, embed_dim)

        if edge_attr_s is None:
            raise ValueError("TransformerConvShortRange requires edge_attr_s, but got None.")

        _, _, edge_s_dim = edge_attr_s.shape
        ...
        if bsz > 1:
            ...
            edge_index_batched = edge_index.repeat(1, bsz) + offsets_repeated # [2, num_edges * bsz]

            # 2. 批处理 edge_attr_s
            # [num_edges, bsz, edge_s_dim] -> [bsz, num_edges, edge_s_dim]
            edge_attr_s_permuted = edge_attr_s.permute(1, 0, 2)
            # [bsz, num_edges, edge_s_dim] -> [bsz * num_edges, edge_s_dim]
            edge_attr_s_batched = edge_attr_s_permuted.reshape(-1, edge_s_dim)

        else:
            edge_index_batched = edge_index
            edge_attr_s_batched = edge_attr_s.squeeze(1) # [num_edges, edge_s_dim]

        # 3. 执行 TransformerConv
        s_out = self.transformer_conv(s_reshaped, edge_index_batched, edge_attr_s_batched)
        s_projected = self.out_proj(s_out)

        # [bsz * seq_len, embed_dim] -> [bsz, seq_len, embed_dim]
        return s_projected.view(bsz, seq_len, -1)

class StandardLongRange(nn.Module):
    """
    使用标准 nn.MultiheadAttention 的长程注意力模块 (V4 架构)。
    这会产生 O(N^2) 的计算复杂度，以消除 DynamicLongRange 的近似损失。

    它封装了 nn.MultiheadAttention 并确保 batch_first=True，
    以匹配 (bsz, seq_len, dim) 的输入/输出格式。
    """
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        self.attn = nn.MultiheadAttention(
            ...
            num_heads=num_heads,
            dropout=dropout,
            batch_first=True # 关键：确保输入/输出是 (bsz, seq_len, dim)
        )
...
class HybridGVP_TransformerConv_Long_Layer(nn.Module):
    """
    V4 混合层:
    1. GVP路径 (局部几何)
    2. 短程路径: TransformerConv (利用边特征)
    3. 长程路径: Standard MHA (O(N^2) 全局)
    """
    def __init__(
            self,
            node_dims,
        ...
        # --- 【V4 架构修改】 ---
        # 5. 实例化新的 TransformerConvShortRange 模块
        # 路径2: V4 TransformerConv 短程
        self.short_range_attn = TransformerConvShortRange(
            embed_dim=node_dims[0],
            num_heads=num_attn_head,
            dropout=drop_rate,
            edge_embed_dim=edge_dims[0] # <-- V4 关键：传入边标量维度
        )

        # 路径3: V4 标准 O(N^2) 长程注意力 (来自上一步)
        self.long_range_attn = StandardLongRange(
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        s_local, v_local = x_local # s_local [nodes, confs, dim]

        # --- 准备注意力路径 ---
        s_res_attn = s_local
        s_norm_attn = self.norm_attn(s_local) #  [nodes, confs, dim]
        s_reshaped = s_norm_attn.permute(1, 0, 2) #  [confs, nodes, dim]

        # --- 【V4 架构修改】 ---
        # 6. 提取 edge_attr 的标量部分 (edge_attr[0])
        # edge_attr[0] 形状: [num_edges, confs, edge_s_dim]
        edge_attr_s_for_attn = edge_attr[0]
        # --- 【V4 架构修改结束】 ---

        # --- 2. & 3. 并行短程与长程路径 ---
        # --- 【V4 架构修改】 ---
        # 7. 将 edge_attr_s_for_attn 传入 short_range_attn
        s_short_out = self.short_range_attn(s_reshaped, edge_index, edge_attr_s_for_attn) # [confs, nodes, dim]
        # --- 【V4 架构修改结束】 ---
        s_long_out = self.long_range_attn(s_reshaped) # [confs, nodes, dim]

        # --- 融合 (V2-MF 逻辑保持不变) ---
        s_local_permuted = s_local.permute(1, 0, 2) # [confs, nodes, dim]

        alpha = 0.3 # 保持 V2-MF 的设置

        s_attn_combined = s_short_out + s_long_out # [confs, nodes, dim]
        s_fused_out = (alpha * s_local_permuted) + ((1 - alpha) * s_attn_combined) # [confs, nodes, dim]
        s_fused_out = s_fused_out.permute(1, 0, 2) # [nodes, confs, dim]
