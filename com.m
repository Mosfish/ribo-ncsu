function data_analysis_comparison()
    % Main function to analyze and compare data from two datasets
    ...
    lngvp_data = read_data_file(lngvp_file);
    my_data = read_data_file(my_file);
    
    if isempty(lngvp_data) || isempty(my_data)
        fprintf('Failed to read data files. Exiting...\n');
        return;
    end
    
    fprintf('Successfully loaded data:\n');
    fprintf('- %s: %d records\n', lngvp_file, size(lngvp_data, 1));
    fprintf('- %s: %d records\n', my_file, size(my_data, 1));
    
    % Analyze data for both datasets
    results_lngvp = analyze_data_by_length(lngvp_data, 'lngvp');
    results_my = analyze_data_by_length(my_data, 'gvpTransformer');
    
    % Display analysis results
    display_results(results_lngvp, results_my);
    
    % Create comparison plots
    create_comparison_plots(results_lngvp, results_my);
...
function display_results(results_lngvp, results_my)
    % Display analysis results in console
    
    fprintf('\n=== DATA ANALYSIS RESULTS ===\n');
    
    % lngvp results
    fprintf('\n%s:\n', results_lngvp.dataset_name);
    fprintf('Length 0-100 range: %d records\n', results_lngvp.count_0_100);
    fprintf('  Recovery - Mean: %.4f, Median: %.4f\n', ...
            results_lngvp.recovery_mean_0_100, results_lngvp.recovery_median_0_100);
    fprintf('  SC Score - Mean: %.4f, Median: %.4f\n', ...
            results_lngvp.sc_score_mean_0_100, results_lngvp.sc_score_median_0_100);
    
    fprintf('Length 100-200 range: %d records\n', results_lngvp.count_100_200);
    fprintf('  Recovery - Mean: %.4f, Median: %.4f\n', ...
            results_lngvp.recovery_mean_100_200, results_lngvp.recovery_median_100_200);
    fprintf('  SC Score - Mean: %.4f, Median: %.4f\n', ...
            results_lngvp.sc_score_mean_100_200, results_lngvp.sc_score_median_100_200);
    
    fprintf('Length 200+ range: %d records\n', results_lngvp.count_200_plus);
    fprintf('  Recovery - Mean: %.4f, Median: %.4f\n', ...
            results_lngvp.recovery_mean_200_plus, results_lngvp.recovery_median_200_plus);
    fprintf('  SC Score - Mean: %.4f, Median: %.4f\n', ...
            results_lngvp.sc_score_mean_200_plus, results_lngvp.sc_score_median_200_plus);
...
function create_comparison_plots(results_lngvp, results_my)
    ...
    % Recovery means
    subplot(2, 2, 1);
    lngvp_recovery_means = [results_lngvp.recovery_mean_0_100, ...
                              results_lngvp.recovery_mean_100_200, ...
                              results_lngvp.recovery_mean_200_plus];
    my_recovery_means = [results_my.recovery_mean_0_100, ...
                         results_my.recovery_mean_100_200, ...
                         results_my.recovery_mean_200_plus];
    
    b1 = bar(x - width/2, lngvp_recovery_means, width, 'FaceColor', [0.2, 0.6, 0.8], 'FaceAlpha', 0.8);
    hold on;
    b2 = bar(x + width/2, my_recovery_means, width, 'FaceColor', [0.8, 0.4, 0.4], 'FaceAlpha', 0.8);
    
    % Add value annotations
    add_value_annotations(b1, lngvp_recovery_means);
    ...
    legend('lngvp', 'gvpTransformer', 'Location', 'best');
    grid on;
    grid minor;
    
    % Recovery medians
    subplot(2, 2, 2);
    lngvp_recovery_medians = [results_lngvp.recovery_median_0_100, ...
                                results_lngvp.recovery_median_100_200, ...
                                results_lngvp.recovery_median_200_plus];
    ...
    b3 = bar(x - width/2, lngvp_recovery_medians, width, 'FaceColor', [0.4, 0.8, 0.4], 'FaceAlpha', 0.8);
    hold on;
    b4 = bar(x + width/2, my_recovery_medians, width, 'FaceColor', [0.9, 0.6, 0.2], 'FaceAlpha', 0.8);
    
    % Add value annotations
    add_value_annotations(b3, lngvp_recovery_medians);
    add_value_annotations(b4, my_recovery_medians);
    ...
    % SC Score means
    subplot(2, 2, 3);
    lngvp_sc_means = [results_lngvp.sc_score_mean_0_100, ...
                        results_lngvp.sc_score_mean_100_200, ...
                        results_lngvp.sc_score_mean_200_plus];
    my_sc_means = [results_my.sc_score_mean_0_100, ...
                   results_my.sc_score_mean_100_200, ...
                   results_my.sc_score_mean_200_plus];
    
    b5 = bar(x - width/2, lngvp_sc_means, width, 'FaceColor', [0.6, 0.4, 0.8], 'FaceAlpha', 0.8);
    hold on;
    b6 = bar(x + width/2, my_sc_means, width, 'FaceColor', [0.9, 0.8, 0.2], 'FaceAlpha', 0.8);
    
    % Add value annotations
    add_value_annotations(b5, lngvp_sc_means);
    add_value_annotations(b6, my_sc_means);
    ...
    % SC Score medians
    subplot(2, 2, 4);
    lngvp_sc_medians = [results_lngvp.sc_score_median_0_100, ...
                          results_lngvp.sc_score_median_100_200, ...
                          results_lngvp.sc_score_median_200_plus];
    my_sc_medians = [results_my.sc_score_median_0_100, ...
                     results_my.sc_score_median_100_200, ...
                     results_my.sc_score_median_200_plus];
    
    b7 = bar(x - width/2, lngvp_sc_medians, width, 'FaceColor', [0.8, 0.5, 0.5], 'FaceAlpha', 0.8);
    hold on;
    b8 = bar(x + width/2, my_sc_medians, width, 'FaceColor', [0.4, 0.7, 0.9], 'FaceAlpha', 0.8);
    
    % Add value annotations
    add_value_annotations(b7, lngvp_sc_medians);
    add_value_annotations(b8, my_sc_medians);
    ...
    % Add main title
   % sgtitle('Data Analysis Comparison Charts', 'FontSize', 16, 'FontWeight', 'bold');
    
    % Save figure
    saveas(gcf, 'data_comparison_analysis.png');
    fprintf('Chart saved as: data_comparison_analysis.png\n');
end
