...
def pdb_to_tensor(
    ...
    if return_sec_struct:
        ...
        if len(sec_struct) != len(sequence):
            print(f"\n\n--- DEBUGGER: Mismatch Found in {filepath} ---")
            print(f"  CPDB Sequence   (Len {len(sequence)}): '{sequence}'")
            print(f"  sec_struct str  (Len {len(sec_struct)}): '{sec_struct}'")
            print(f"--- END DEBUGGER ---")
        # --- 探针结束 ---
        # --- V5 热修复：开始 ---
        # 如果 sec_struct 工具返回空，而序列不为空 (常见于短序列)
        # 我们用 '.' (未配对) 填充，以防止断言崩溃
        if len(sec_struct) == 0 and len(sequence) > 0:
            print(f"    WARNING: {filepath.split('/')[-1]} - sec_struct empty for seq len {len(sequence)}. Filling with '.'")
