...
def get_model(config):
    ...
    model_kwargs = {
        ...
        # [V8 FIX] Pass new Graphormer/V8 parameters from config to model
        'heads': config.heads,
        'max_degree': config.max_degree,
        'degree_emb_dim': config.degree_emb_dim,
    }

    # [V8] Add max_path_distance if available in config
    if hasattr(config, 'max_path_distance'):
        model_kwargs['max_path_distance'] = config.max_path_distance
